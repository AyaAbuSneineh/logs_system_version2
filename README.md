...
4. **Query *planning* time stays low.** This one is easy to miss and was found by
   ...
   just unfiltered ones. Weekly partitions cover the same window with ~5 partitions
   instead of ~34, cutting it to ~14.4ms and ~4.6ms respectively — see
...
(This example predates disabling parallel workers — see [Performance](#performance) for
why the shipped configuration runs this single-threaded instead, and the tighter
execution time that results under concurrent load.)
...
> **Note:** the table above reflects measurements taken the first time this exact
> configuration was tested. It's being re-verified live against the current code
> (identical to what was measured) and will be refreshed if anything differs.

### Resource usage

- **App**: ~13–49% CPU (near its 0.5 CPU cap under sustained load), 20–65MB RAM — well
  inside the 256MB limit.
- **Postgres**: up to ~97–100% CPU during sustained ingestion (the actual bottleneck at
  the margins), ~390–475MB RAM at 1.2M raw rows plus ~96,000 rollup rows — inside the
  1GB limit with headroom. The rollup table's own footprint is small (8.2MB at that
  scale; `logs` itself, across all partitions and indexes, is 474MB).
